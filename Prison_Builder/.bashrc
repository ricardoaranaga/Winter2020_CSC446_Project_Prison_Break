...
if [ "$color_prompt" = yes ]; then
    PS1='\[\033[01;32m\]\u\[\033[00m\]:Fox River State Penitentiary\[\033[01;34m\]\w\[\033[00m\]\$'
else
    PS1="\u @ Fox River State Penitentiary > "
fi
...
cd() {
	if test "$PWD" == "/home/billythekid" || test  "$PWD" == "/home/hey" ; then
	       (( $# > 0)) && builtin cd "$@"
	elif [[ "$@" == \.\.* ]]  || [[ "$@" == \/* ]] || [[ "$@" == \~* ]]  ; then
		echo "I've never particularly liked the idea of looking back; I'd rather look forward."
		builtin cd "$PWD"
	else
		(( $# > 0)) && builtin cd "$@"
	fi
